...
void batter_step(Entity* self) {
    ObjBatter* this = self->data;
    ...
    self->position.x += (float) horizontal_direction * this->speed;
    self->position.y += (float) vertical_direction * this->speed;

    const bool is_moving = horizontal_direction != 0 || vertical_direction != 0;
    if (is_moving) {
        if (vertical_direction != 0) {
            if (vertical_direction == 1) this->direction = DOWN;
            else this->direction = UP;
        } else {
            if (horizontal_direction == 1) this->direction = RIGHT;
            else this->direction = LEFT;
        }

        self->image_index += self->image_speed;
        if (self->image_index > self->image_number) self->image_index -= self->image_number;
    } else self->image_index = 0;
}

void batter_draw(Entity* self) {
    ObjBatter* data = self->data;
}
...
int main() {
    ...
    entity_register(OBJ_BATTER, (Entity_Methods) {
        .step = batter_step,
        .draw = batter_draw
    });

    instance_create(OBJ_BATTER);
